...
export const metadata: Metadata = {
  ...
  description:
    'A visual map of relationships between essays, field notes, projects, and shelf items.',
};
...
export default function ConnectionsPage() {
  ...
  const edges = threadPairs
    .map((pair) => {
      // Source node type matches the pair type
      const sourceId =
        pair.type === 'essay'
          ? `essay-${pair.fromSlug}`
      ...
      // Targets are always essays (all current thread pair types)
    ...
    <>
      <section className="py-8">
        ...
        <h1 className="font-title text-3xl md:text-4xl font-bold mb-2 flex items-center gap-3">
          <DrawOnIcon name="gears" size={32} color="var(--color-teal)" />
          Connection Map
        </h1>
        <p className="text-ink-secondary mb-5">
          How ideas, notes, and references connect across this site.
        </p>
      </section>
